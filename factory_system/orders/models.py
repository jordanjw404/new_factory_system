...
class Order(models.Model):
    ...
    delivery_date = models.DateField(
        null=True, blank=True, help_text="Expected delivery or collection date"
    )
    is_collection = models.BooleanField(default=False)
    order_type = models.CharField(
        max_length=20, choices=OrderType.choices, default=OrderType.KITCHEN
    )
    status = models.CharField(
        max_length=30, choices=Status.choices, default=Status.NO_PAPERWORK
    )
    ...
    send_to_production = models.BooleanField(
        default=False, help_text="Tick to generate a production order."
    )
