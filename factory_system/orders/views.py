...
from production.utils import compute_stage_targets, create_production_stage

from .filters import OrderFilter
from .forms import DeliveryDateUpdateForm, OrderForm
...
def order_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if original_delivery_date != new_delivery:
                ...
                request.session["original_delivery_date"] = (
                    original_delivery_date.isoformat()
                )
    ...
    return render(
        ...
        {"form": form, "edit_mode": True, "order": order},
    )
...
def delivery_date_update(request, pk):
            ...
            if update_choice == "all":
                ...
                if hasattr(order, "productionstage"):
                    ...
                    for stage, target_date in targets.items():
                        setattr(
                            order.productionstage, f"{stage}_target_date", target_date
                        )
    ...
    return render(
        request,
        "orders/delivery_date_update.html",
        {
            "form": form,
            "order": order,
