...
def compute_stage_targets(
    delivery_date: date, offsets: Optional[Dict[str, int]] = None
) -> Dict[str, date]:
    """
...
def create_production_stage(
    order: Any, offsets: Optional[Dict[str, int]] = None
) -> ProductionStage:
    """
    ...
    if not created:
        ...
        for stage in DEFAULT_OFFSETS:
            ...
            if (
                not getattr(obj, status_attr)
                or getattr(obj, status_attr) == "NOT_STARTED"
            ):
...
def update_stage_status(
    production_stage: ProductionStage, stage_name: str, new_status: str
) -> None:
    """
