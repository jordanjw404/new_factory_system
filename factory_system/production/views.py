...
def production_list(request):
    ...
    badge_colors = {
        st.id: {f: get_badge_color(getattr(st, f"{f}_status")) for f in STATUS_FIELDS}
        for st in stages
    }
    return render(
        request,
...
def production_detail(request, pk):
    ...
    badge_colors = {
        f: get_badge_color(getattr(stage, f"{f}_status")) for f in STATUS_FIELDS
    }
    return render(
        request,
...
def production_create(request, order_pk=None):
    ...
    return render(
        request,
        "production/production_create.html",
        {
            "orders": orders,
        },
    )
